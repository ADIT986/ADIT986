--// GUI + Config Setup local ScreenGui = Instance.new("ScreenGui") local Frame = Instance.new("Frame") local Title = Instance.new("TextLabel") local FishingButton = Instance.new("TextButton") local AutoShake = Instance.new("TextButton") local AutoReel = Instance.new("TextButton") local AutoSell = Instance.new("TextButton") local Config = { AutoCast = false, AutoShake = false, AutoReel = false, AutoSell = false }

-- Parent to PlayerGui ScreenGui.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui") Frame.Parent = ScreenGui Frame.Size = UDim2.new(0, 250, 0, 300) Frame.Position = UDim2.new(0.02, 0, 0.2, 0) Frame.BackgroundColor3 = Color3.fromRGB(20, 20, 20) Frame.BorderSizePixel = 2 Frame.Active = true Frame.Draggable = true
